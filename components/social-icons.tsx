...
const FacebookIcon = ({ className }: { className?: string }) => (
  <svg 
    viewBox="0 0 24 24" 
    fill="currentColor" 
    className={cn('w-[21px] h-[21px]', className)}
    aria-hidden="true"
  >
...
export function getSocialIcon(platform: SocialPlatform, className?: string) {
  const icons: Record<SocialPlatform, React.ReactNode> = {
    ...
    instagram: <Instagram className={cn('w-[21px] h-[21px]', className)} />,
    youtube: <Youtube className={cn('w-[21px] h-[21px]', className)} />,
...
const iconSizeClasses = {
  sm: 'w-4 h-4',
  md: 'w-[21px] h-[21px] sm:w-[25px] sm:h-[25px]',
  lg: 'w-[25px] h-[25px]',
};
